<script lang="ts">
  ...
  import { faBoltLightning, faGear, faRightFromBracket, faUser, faWallet } from '@fortawesome/free-solid-svg-icons';
  ...
  async function modifyLists() {
    ...
    for (let i = 0; i < toggleListArr.length; i++) {
      ...
      if (newEvents.find((t) => t[1] === `${event.kind}:${event.author.pubkey}:${event.replaceableDTag()}`)) {
        newEvents.forEach((e) => {
        ...
        <div class="flex flex-col gap-2">
          {#each lists as list, index}
            <div class="flex gap-14">
              ...
              <input class="self-center" type="checkbox" on:change={() => toggleList(list.replaceableDTag())} checked={list.getMatchingTags("a").find((t) => t[1] == `${event.kind}:${event.author.pubkey}:${event.replaceableDTag()}`) ? true : false} />
            </div>
            {#if index === 0}
              <hr />
            {/if}
